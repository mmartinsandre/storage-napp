...
1. Abra o Postman.
2. Crie uma nova requisição `POST` para `http://localhost:8080/login`.
3. No corpo da requisição, selecione `raw` e escolha `JSON`.
4. Adicione o seguinte JSON com as credenciais de login:
...
5. Envie a requisição. Você deve receber uma resposta com o token JWT, algo assim:
...
### Passo 2: Utilizar o Token JWT

1. Copie o valor do token (sem as aspas).
2. Crie uma nova requisição no Postman para qualquer um dos endpoints protegidos, por exemplo, `GET http://localhost:8080/produto`.
3. Vá para a aba `Headers` da requisição.
4. Adicione um novo header com a chave `Authorization` e o valor `Bearer <seu_token_jwt>`, substituindo `<seu_token_jwt>` pelo token que você copiou.
